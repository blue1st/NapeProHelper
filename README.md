...
**NapePro Helper** は、トラックボール内蔵の左右分離型エルゴノミックキーボード **Keychron Nape Pro** のためのシステムトレイ・ヘルパーアプリケーションです。

Keychron Nape Pro は、左右セパレート構造による自然なタイピング姿勢と、中央に配置されたトラックボールによるスムーズなポインタ操作を両立したエルゴノミックキーボードです。

本アプリケーションはシステムトレイ（メニューバー）に常駐し、手のホームポジションやキーボードの設置角度に合わせたトラックボールの操作角度補正（**OctaShift**）、DPI感度調整、スクロール・ジェスチャーモードの切り替え、およびアクティブレイヤーやキーマップ状態のリアルタイム可視化を強力にサポートします。
...
- 🧭 **OctaShift 角度調整 (OctaShift Angle Tuning)**  
  キーボードの開き具合や手の角度に合わせて、レイヤーごとにトラックボールのカーソル移動角度（0°〜315° の8方向）をリアルタイムに調整・補正。
